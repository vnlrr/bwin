...
local function shadow(id, x, y, w, h, z, corner, spread, strength, layers)
    layers = layers or 5
    spread = spread or 9
    strength = strength or 0.16
    for i = 1, layers do
        local g = i * (spread / layers)
        local op = strength * (1 - (i - 1) / layers)
        rect(id .. ".sh" .. i, x - g, y - g + 2.5, w + g * 2, h + g * 2,
            ShadowColor, op, (z or 1) - 1, (corner or 0) + g)
    end
end
-- 1px top inner-highlight, brighter on hover, for a "lit from above" bevel.
local HiColor = Color3.fromRGB(255, 255, 255)
local function innerHi(id, x, y, w, z, amt)
    line(id, x + 5, y + 1, x + w - 5, y + 1, HiColor, amt or 0.05, z)
end
...
local function processEl(el, idp, x, y, w, h, dt, z, block)
    ...
    if el.kind == "button" then
        el.press = el.press or newSpring(0, 26)
        el.press.goal = (hovered and Input.down) and 1 or 0
        springStep(el.press, dt)
        if el.press.v > 0.01 then rect(idp .. ".pr", x, y, w, h, ShadowColor, 0.12 * el.press.v, z + 1, 6) end
...
local function renderOverlay(dt)
    ...
    if ov.kind == "dropdown" then
        ...
        shadow("ov", ov.x, ov.y, panelW, panelH, Z, 5, 11, 0.18 * a, 5)
        rect("ov.bg", ov.x, ov.y, panelW, panelH, Theme.OverlayBg, OP.Overlay * a, Z, 6)
        outline("ov.bd", ov.x, ov.y, panelW, panelH, Theme.OverlayBorder, 0.7 * a, Z + 1, 6)
        ...
        for i = 1, visN do
            ...
            if opt ~= nil then
                ...
                if selected then circle("ov.m" .. i, ov.x + panelW - barW - 12, ry + rowH / 2 - 1, 3, Theme.Accent, a, Z + 3) end
    ...
    elseif ov.kind == "colorpicker" then
        ...
        shadow("ovc", px, py, panelW, panelH, Z, 5, 12, 0.20, 6)
        rect("ov.bg", px, py, panelW, panelH, Theme.OverlayBg, OP.Overlay, Z, 6)
        outline("ov.bd", px, py, panelW, panelH, Theme.OverlayBorder, 0.7, Z + 1, 6)
...
local function renderWindow(dt)
    ...
    local gripOp = gripHover and 0.9 or 0.4
    local gripCol = gripHover and Theme.Accent or Theme.SubText
    local gbx, gby = win.x + win.w - 5, win.y + win.h - 5
    circle("win.grip1", gbx, gby, 1.4, gripCol, gripOp, 62)
    circle("win.grip2", gbx - 5, gby, 1.4, gripCol, gripOp, 62)
    circle("win.grip3", gbx, gby - 5, 1.4, gripCol, gripOp, 62)

    shadow("win", win.x, win.y, win.w, win.h, 9, 9, 14, 0.18, 6)
        ...
        for _, g in ipairs(tab.groups) do
            if g.title then
                ...
                if cy + hh > cvy and (cy < cvy or cy + hh <= viewBottom + 1) then
                    rect("t" .. State.ActiveTab .. "gd" .. idx, cx + 2, cy + 6, 3, 14, Theme.Accent, 1, 22, 2)
                    text("t" .. State.ActiveTab .. "g" .. idx, g.title, cx + 12, cy + 10, 15, Theme.Text, 22)
                    line("t" .. State.ActiveTab .. "gl" .. idx, cx + 2, cy + 27, cx + cw - 2, cy + 27, Theme.RailLine, 0.45, 22)
                end
                cy = cy + hh + gap
            end
...
local function renderDialog(dt)
    ...
    shadow("dlg", bx, by, bw, bh, 150, 8, 16, 0.22 * a, 6)
    rect("dlg.bg", bx, by, bw, bh, Theme.OverlayBg, a, 150, 8)
    innerHi("dlg.hi", bx, by, bw, 151, 0.05 * a)
...
local function renderBubble(dt)
    ...
    shadow("bub", bp.x, bp.y, pw, ph, 119, 9, 10, 0.18, 5)
    rect("bub.bg", bp.x, bp.y, pw, ph, Theme.WindowBg, OP.Window, 120, 9)
    innerHi("bub.hi", bp.x, bp.y, pw, 121, 0.05)
